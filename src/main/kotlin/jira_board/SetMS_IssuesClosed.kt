...
/**
 * Method to modify the state of a list of issues from "done" to "closed"
 * - two transitions :
 *   741 : test
 *   821 : done
 *   811 : close
    ...
    for(x in issueList){
        ...
        if ((cl.size == 1) || ((cl.size == 2) && (cl.contains("ms-configuration"))) ) {
            ...
            if (state == "Entwicklung abgeschlossen") {
                ...
                j.setIssueState(x, "821")
                if (j.getIssueState(x) == "Erledigt") {
                    j.addComment(x, "Done! by mcbstest@bamboo")
                }
                j.setIssueState(x, "811")
                if (j.getIssueState(x) == "Geschlossen") {
                    j.addComment(x, "Closed! by mcbstest@bamboo")
                }
                ...
                logger.info("Issue : $x : Ausgangsstatus unpassend !!")
            }
        } else {
